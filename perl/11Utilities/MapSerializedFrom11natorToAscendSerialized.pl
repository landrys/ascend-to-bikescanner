...
sub processNewEntries {
    ...
    my $maxAscendSerializedId = getMaxAscendSerializedId();

    # get any new ones from the serialized table 
    my $statement = $elevenProdDB->prepare("select * from serialized where id>? and serial is not null and lc_deleted is null limit 100");
    if ( !defined($maxAscendSerializedId) || ($maxAscendSerializedId eq 'NULL') )  {
	    $maxAscendSerializedId = 0;
    }
    my $count = $statement->execute($maxAscendSerializedId);
